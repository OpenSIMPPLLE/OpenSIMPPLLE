...
# Need to add acres field to result fc and set its value.
...
inputRaster = gp.GetParameterAsText(2)
...
cellsizeX = gp.GetRasterProperties_management(inputRaster,"CELLSIZEX")
cellsizeY = gp.GetRasterProperties_management(inputRaster,"CELLSIZEY")

if (cellsizeX != cellsizeY):
    AddMsgAndPrint("CELLSIZEX and CELLSIZEY must be equal on the input raster.\n Quiting.",2)
    sys.exit(2)
...
def buildTempGDB():
    ...
    # This will make sure that the input raster has an object id field
    tmpRaster = makepath(gdb,"inraster")
    gp.CopyRaster(inputRaster,tmpRaster)
    inputRaster = tmpRaster
...
def main():
    ...
    AddMsgAndPrint("Adding field to temp raster.")        
    gp.AddField_management(inputRaster, "CELLID", "LONG")

    AddMsgAndPrint("Setting temp field of raster to OBJECTID")        
    rasterOIDFields = gp.ListFields(inputRaster,"*","OID")
    rasterOIDField = rasterOIDFields.Next()
    OIDFieldExpr = "[" + rasterOIDField.Name + "]"

    gp.CalculateField_management(inputRaster, "CELLID", OIDFieldExpr, "VB")
    ...
    gp.RasterToPoint_conversion(inputRaster, rasterpts, "CELLID")
    ...
    AddMsgAndPrint("Copy data from Raster Attribute Table to Polygon Feature Class Table")        
    gp.joinfield_management (resultFC, "GRID_CODE", inputRaster, "CELLID")
    
    AddMsgAndPrint("Cleanup")    
    # Cleanup Temp Data
    gp.DeleteField_management (resultFC, "GRID_CODE")
    gp.DeleteField_management (resultFC, "GRIDCODE")
    gp.DeleteField_management (resultFC, "CELLID")
