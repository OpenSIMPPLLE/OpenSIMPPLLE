...
/**
 * This class contains methods for a Process Reports, a type of Report.
 *
 ...
 */
  private void summaryReportNormal(PrintWriter fout) {
    ...
    fout.println();
    fout.println(Formatting.padLeft("ACRES OF PROCESSES (rounded)",31));
    ...
    for(int i=0;i<=timeSteps;i += 5) {
      ...
      for(int j=0;j<summaryProcesses.length;j++) {
        ...
        fout.print(Formatting.fixedField(str,41,true));

        for(int k=i;k<=lastTime;k++) {
          float acres = Area.getFloatAcres(acresData[k]);
          fout.print(Formatting.fixedField(nf.format(acres),9));
        }
        fout.println();
      }
      fout.println();
