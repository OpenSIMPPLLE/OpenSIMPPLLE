...
public abstract class Wsbw {
   ...
   * @param evu is an Evu.
   */
  public static int computeSusceptibilityIndex (simpplle.comcode.zone.EastsideRegionOne zone, simpplle.comcode.element.Evu evu) {
    float index = 0.0f;
  ...
  public static int computeSusceptibilityIndex (simpplle.comcode.zone.Teton zone, simpplle.comcode.element.Evu evu) {
    float index = 0.0f;
  ...
  public static int computeSusceptibilityIndex (simpplle.comcode.zone.NorthernCentralRockies zone, simpplle.comcode.element.Evu evu) {
    float index = 0.0f;
  ...
  private static boolean oneOfHosts(simpplle.comcode.zone.WestsideRegionOne zone, simpplle.comcode.element.Evu evu) {
  ...
  private static boolean oneOfHosts(simpplle.comcode.zone.EastsideRegionOne zone, simpplle.comcode.element.Evu evu) {
    return oneOfHostsEast(evu);
  }
  private static boolean oneOfHosts(simpplle.comcode.zone.Teton zone, simpplle.comcode.element.Evu evu) {
    return oneOfHostsEast(evu);
  }
  private static boolean oneOfHosts(simpplle.comcode.zone.NorthernCentralRockies zone, simpplle.comcode.element.Evu evu) {
    return oneOfHostsEast(evu);
  }
  private static boolean oneOfHostsEast(simpplle.comcode.element.Evu evu) {
    Species species = (Species)evu.getState(SimpplleType.SPECIES);
  ...
  private static float percentHostIndex(WestsideRegionOne zone, simpplle.comcode.element.Evu evu) {
    float   index = 0.0f;
    Species species = (Species)evu.getState(SimpplleType.SPECIES);
  ...
  private static float percentHostIndex(simpplle.comcode.zone.EastsideRegionOne zone, simpplle.comcode.element.Evu evu) {
    return percentHostIndexEast(evu);
  }
  private static float percentHostIndex(simpplle.comcode.zone.Teton zone, simpplle.comcode.element.Evu evu) {
    return percentHostIndexEast(evu);
  }
  private static float percentHostIndex(simpplle.comcode.zone.NorthernCentralRockies zone, simpplle.comcode.element.Evu evu) {
    return percentHostIndexEast(evu);
  }
  private static float percentHostIndexEast(simpplle.comcode.element.Evu evu) {
    float   index   = 0.0f;
    Species species = (Species)evu.getState(SimpplleType.SPECIES);
  ...
  private static float percentClimateHostIndex(EastsideRegionOne zone, simpplle.comcode.element.Evu evu) {
    return percentClimateHostIndexEast(evu);
  }
  private static float percentClimateHostIndex(simpplle.comcode.zone.Teton zone, simpplle.comcode.element.Evu evu) {
    return percentClimateHostIndexEast(evu);
  }
  private static float percentClimateHostIndex(simpplle.comcode.zone.NorthernCentralRockies zone, simpplle.comcode.element.Evu evu) {
    return percentClimateHostIndexEast(evu);
  }
  private static float percentClimateHostIndexEast(simpplle.comcode.element.Evu evu) {
    float                index     = 0.0f;
    Species              species   = (Species)evu.getState(SimpplleType.SPECIES);
    HabitatTypeGroupType groupType = evu.getHabitatTypeGroup().getType();
  ...
  private static float densityIndex(simpplle.comcode.element.Evu evu) {
    float   index   = 0.0f;
  ...
  private static float densityIndex(simpplle.comcode.zone.WestsideRegionOne zone, simpplle.comcode.element.Evu evu) {
    return densityIndex(evu);
  }

  private static float densityIndex(simpplle.comcode.zone.EastsideRegionOne zone, simpplle.comcode.element.Evu evu) {
    return densityIndex(evu);
  }
  private static float densityIndex(Teton zone, simpplle.comcode.element.Evu evu) {
    return densityIndex(evu);
  }
  private static float densityIndex(simpplle.comcode.zone.NorthernCentralRockies zone, simpplle.comcode.element.Evu evu) {
    return densityIndex(evu);
  }

  private static float structureIndex(RegionalZone zone, simpplle.comcode.element.Evu evu) {
    float     index     = 0.0f;
  ...
  private static float structureIndex(simpplle.comcode.zone.WestsideRegionOne zone, simpplle.comcode.element.Evu evu) {
    return structureIndex((RegionalZone)zone,evu);
  }

  private static float structureIndex(simpplle.comcode.zone.EastsideRegionOne zone, simpplle.comcode.element.Evu evu) {
    return structureIndex((RegionalZone)zone,evu);
  }
  private static float structureIndex(simpplle.comcode.zone.Teton zone, simpplle.comcode.element.Evu evu) {
    return structureIndex((RegionalZone)zone,evu);
  }
  private static float structureIndex(simpplle.comcode.zone.NorthernCentralRockies zone, simpplle.comcode.element.Evu evu) {
    return structureIndex((RegionalZone)zone,evu);
  }

  private static float standVigorIndex(RegionalZone zone, simpplle.comcode.element.Evu evu) {
    float                index     = 0.0f;
  ...
  private static float standVigorIndex(simpplle.comcode.zone.WestsideRegionOne zone, simpplle.comcode.element.Evu evu) {
    return standVigorIndex((RegionalZone)zone,evu);
  }

  private static float standVigorIndex(simpplle.comcode.zone.EastsideRegionOne zone, simpplle.comcode.element.Evu evu) {
    return standVigorIndex((RegionalZone)zone,evu);
  }
  private static float standVigorIndex(simpplle.comcode.zone.Teton zone, simpplle.comcode.element.Evu evu) {
    return standVigorIndex((RegionalZone)zone,evu);
  }
  private static float standVigorIndex(simpplle.comcode.zone.NorthernCentralRockies zone, simpplle.comcode.element.Evu evu) {
    return standVigorIndex((RegionalZone)zone,evu);
  }

  private static float maturityIndex(RegionalZone zone, simpplle.comcode.element.Evu evu) {
    float     index     = 0.0f;
    SizeClass sizeClass = (SizeClass)evu.getState(SimpplleType.SIZE_CLASS);
  ...
  private static float maturityIndex(simpplle.comcode.zone.WestsideRegionOne zone, simpplle.comcode.element.Evu evu) {
    return maturityIndex((RegionalZone)zone,evu);
  }
  private static float maturityIndex(simpplle.comcode.zone.EastsideRegionOne zone, simpplle.comcode.element.Evu evu) {
    return maturityIndex((RegionalZone)zone,evu);
  }
  private static float maturityIndex(simpplle.comcode.zone.Teton zone, simpplle.comcode.element.Evu evu) {
    return maturityIndex((RegionalZone)zone,evu);
  }
  private static float maturityIndex(simpplle.comcode.zone.NorthernCentralRockies zone, simpplle.comcode.element.Evu evu) {
    return maturityIndex((RegionalZone)zone,evu);
  }

  private static float siteClimateIndex(simpplle.comcode.zone.WestsideRegionOne zone, simpplle.comcode.element.Evu evu) {
    float                index;
  ...
  private static float siteClimateIndex(simpplle.comcode.zone.EastsideRegionOne zone, simpplle.comcode.element.Evu evu) {
    return siteClimateIndexEast(evu);
  }
  private static float siteClimateIndex(simpplle.comcode.zone.Teton zone, simpplle.comcode.element.Evu evu) {
    return siteClimateIndexEast(evu);
  }
  private static float siteClimateIndex(simpplle.comcode.zone.NorthernCentralRockies zone, simpplle.comcode.element.Evu evu) {
    return siteClimateIndexEast(evu);
  }
  private static float siteClimateIndexEast(simpplle.comcode.element.Evu evu) {
    float                index = 0.0f;
    HabitatTypeGroupType groupType;
  ...
  private static float regionalClimateIndex(simpplle.comcode.zone.WestsideRegionOne zone, simpplle.comcode.element.Evu evu) {
    return regionalClimateIndex();
  }

  private static float regionalClimateIndex(simpplle.comcode.zone.EastsideRegionOne zone, simpplle.comcode.element.Evu evu) {
    return regionalClimateIndex();
  }
  private static float regionalClimateIndex(simpplle.comcode.zone.Teton zone, simpplle.comcode.element.Evu evu) {
    return regionalClimateIndex();
  }
  private static float regionalClimateIndex(simpplle.comcode.zone.NorthernCentralRockies zone, simpplle.comcode.element.Evu evu) {
    return regionalClimateIndex();
  }
  ...
  private static float characterOfAdjacentIndex(simpplle.comcode.zone.EastsideRegionOne zone, simpplle.comcode.element.Evu evu) {
    return characterOfAdjacentIndexEast(evu);
  }
  private static float characterOfAdjacentIndex(simpplle.comcode.zone.Teton zone, simpplle.comcode.element.Evu evu) {
    return characterOfAdjacentIndexEast(evu);
  }
  private static float characterOfAdjacentIndex(simpplle.comcode.zone.NorthernCentralRockies zone, simpplle.comcode.element.Evu evu) {
    return characterOfAdjacentIndexEast(evu);
  }

  private static float characterOfAdjacentIndexEast(simpplle.comcode.element.Evu evu) {
    int   immatureOneThird = 0;
    int   immatureOneHalf  = 0;
    int   immatureOverHalf = 0;
    int   matureOneThird   = 0;
   ...
   */
  public static boolean doSpread(RegionalZone zone, Process p, simpplle.comcode.element.Evu evu) {
    int prob;
    ...
    if (evuProb != simpplle.comcode.element.Evu.L &&
