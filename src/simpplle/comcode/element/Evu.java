...
public final class Evu extends NaturalElement implements Externalizable {
  ...
  private ArrayList<simpplle.comcode.element.Roads>               assocRoadUnits;
  private ArrayList<simpplle.comcode.element.Trails>              assocTrailUnits;
...
//  private HashMap<Lifeform,ArrayList<VegSimStateData>> vegStateNew;
  ...
  private simpplle.comcode.element.Roads.Status     roadStatus;
 ...
 */
  ...
  public static class RoadUnitData {
    public simpplle.comcode.element.Roads road;
    public Evu   evu;
  }
 ...
 */
  public static class TrailUnitData {
    public simpplle.comcode.element.Trails trail;
    public Evu    evu;
  }
 ...
 */
  public simpplle.comcode.element.Roads.Status getRoadStatusNew() {
   ...
   */
  public simpplle.comcode.element.Roads.Status getRoadStatusNew(int ts) {
    ArrayList<simpplle.comcode.element.Roads> roads = this.getAssociatedRoadUnits();
    simpplle.comcode.element.Roads road = null;
 ...
 * @return
 */
  public simpplle.comcode.element.Trails.Status getTrailStatus() {
    Simulation simulation = Simpplle.getCurrentSimulation();
    int ts = 0;
    if (simulation != null && simulation.isSimulationRunning()) {
      ts = simulation.getCurrentTimeStep();
   ...
   */
  public simpplle.comcode.element.Trails.Status getTrailStatus(int ts) {
    ArrayList<simpplle.comcode.element.Trails> trails = this.getAssociatedTrailUnits();
    simpplle.comcode.element.Trails trail = null;
    ...
    if (trail != null) {
      ...
      return simpplle.comcode.element.Trails.Status.UNKNOWN;
    }
  }
/**
 ...
 */
  ...
  public Evu() {
    ...
    ownership     = null;
    roadStatus    = simpplle.comcode.element.Roads.Status.UNKNOWN;
    ignitionProb  = 0;
 ...
 */
  public ArrayList<simpplle.comcode.element.Roads> getAssociatedRoadUnits() { return assocRoadUnits; }
  /**
   ...
   */
  public void addAssociatedRoadUnit(simpplle.comcode.element.Roads roadUnit)  {
    if (assocRoadUnits == null) { assocRoadUnits = new ArrayList<simpplle.comcode.element.Roads>(); }
  ...
  private boolean hasAssociatedRoadUnit(RoadUnitData data, boolean onlyOpen) {
    ...
    for(int i=0; i<assocRoadUnits.size(); i++) {
      simpplle.comcode.element.Roads road = assocRoadUnits.get(i);
      if (onlyOpen && road.getSimStatus() != simpplle.comcode.element.Roads.Status.OPEN) {
        continue;
      }
      data.road = road;
      return true;
 ...
 */
  public ArrayList<simpplle.comcode.element.Trails> getAssociatedTrailUnits() { return assocTrailUnits; }
  /**

   *  If the associated road unit arraylist does not contain passed road, adds the existing road to the arraylist.
   * @param trailUnit trail to be added
   */
  public void addAssociatedTrailUnit(simpplle.comcode.element.Trails trailUnit)  {
    if (assocTrailUnits == null) { assocTrailUnits = new ArrayList<simpplle.comcode.element.Trails>(); }
 ...
 */
  private boolean hasAssociatedTrailUnit(TrailUnitData data, boolean onlyOpen) {
    ...
    for(int i=0; i<assocTrailUnits.size(); i++) {
      simpplle.comcode.element.Trails trail = assocTrailUnits.get(i);
      if (onlyOpen && trail.getSimStatus() != simpplle.comcode.element.Trails.Status.OPEN) {
        continue;
      }
      data.trail = trail;
      return true;
 ...
 */
  ...
  public void setAcres(float val) {
    acres = Math.round(val * simpplle.comcode.utility.Utility.pow(10,Area.getAcresPrecision()));
  }
/**
 ...
 */
  public static void findRoadUnits() {
    simpplle.comcode.element.Roads[] allUnits = Simpplle.getCurrentArea().getAllRoads();
 ...
 */
  public double findDistanceNearestRoad() {
    ...
    for (int i=0; i<list.size(); i++) {
      data = list.get(i);
      if (data != null && data.road != null && (data.road.getSimStatus(ts) == simpplle.comcode.element.Roads.Status.OPEN)) {
        break;
      }
      data = null;
    }
 ...
 */
  ...
  public void updateNearestRoad() {
    ...
    for (int i=0; i<list.size(); i++) {
      ...
      if (data.road.getSimStatus(ts) == simpplle.comcode.element.Roads.Status.OPEN) {
        return;
      }
    }
 ...
 */
  private double distanceToRoad(RoadUnitData roadData, boolean onlyOpen) {
    ...
    for (int i=0; i<roadUnits.size(); i++) {
      ...
      if (onlyOpen && tmp.road.getSimStatus() != simpplle.comcode.element.Roads.Status.OPEN) { continue; }
 ...
 */
  private Evu findClosestRoadEvu(simpplle.comcode.element.Roads road) {
 ...
 */
  public void openRoads() {
    changeRoadsStatus(simpplle.comcode.element.Roads.Status.OPEN);
  }
  /**
   * Changes the road status to closed.
   */
  public void closeRoads() {
    changeRoadsStatus(simpplle.comcode.element.Roads.Status.CLOSED);
  }
  /**
   ...
   */
  public void changeRoadsStatus(simpplle.comcode.element.Roads.Status status) {
 ...
 */
  public void openTrails() {
    changeTrailsStatus(simpplle.comcode.element.Trails.Status.OPEN);
  }
  /**
   * Change Trail status to Closed.
   */
  public void closeTrails() {
    changeTrailsStatus(simpplle.comcode.element.Trails.Status.CLOSED);
  }
  /**
   ...
   */
  public void changeTrailsStatus(simpplle.comcode.element.Trails.Status status) {
 ...
 */
  public static void findTrailUnits() {
    simpplle.comcode.element.Trails[] allUnits = Simpplle.getCurrentArea().getAllTrails();
 ...
 */
  public double findDistanceNearestTrail() {
    ...
    for (int i=0; i<list.size(); i++) {
      data = list.get(i);
      if (data != null && data.trail != null && (data.trail.getSimStatus(ts) == simpplle.comcode.element.Trails.Status.OPEN)) {
        break;
      }
      data = null;
    }
  ...
  public void updateNearestTrail() {
    ...
    for (int i=0; i<list.size(); i++) {
      ...
      if (data.trail.getSimStatus(ts) == simpplle.comcode.element.Trails.Status.OPEN) {
        return;
      }
    }
 ...
 */
  private double distanceToTrail(TrailUnitData trailData, boolean onlyOpen) {
    ...
    for (int i=0; i<trailUnits.size(); i++) {
      ...
      if (onlyOpen && tmp.trail.getSimStatus() != simpplle.comcode.element.Trails.Status.OPEN) { continue; }
 ...
 */
  private Evu findClosestTrailEvu(simpplle.comcode.element.Trails trail) {
 ...
 */
  public float getFloatProcessProb(ProcessType p) {
    return ( (float)getProcessProb(p) / (float) simpplle.comcode.utility.Utility.pow(10,2) );
  }
 ...
 */
  public HashMap<SimpplleType, simpplle.comcode.utility.MyInteger> getSpeciesFrequency() throws SimpplleError {
   ...
   * @throws SimpplleError caught in GUI
   */
  public HashMap<SimpplleType, simpplle.comcode.utility.MyInteger> getSizeClassFrequency() throws SimpplleError {
    MultipleRunSummary mrSummary = Simpplle.getCurrentSimulation().getMultipleRunSummary();
  ...
  public HashMap<SimpplleType, simpplle.comcode.utility.MyInteger> getDensityFrequency() throws SimpplleError {
    MultipleRunSummary mrSummary = Simpplle.getCurrentSimulation().getMultipleRunSummary();
   ...
   * @throws SimpplleError caught in GUI
   */
  public HashMap<SimpplleType, simpplle.comcode.utility.MyInteger> getProcessFrequency() throws SimpplleError {
    MultipleRunSummary mrSummary = Simpplle.getCurrentSimulation().getMultipleRunSummary();
      ...
      for (int i=0; i<seasons.length; i++) {
        ...
        for (Lifeform lifeform : getSortedLifeforms(ts,seasons[i])) {
          ...
          if (ts == 0  || roadsDone) {
            ...
            simpplle.comcode.element.ManmadeElement unit = null;
            if (distRoadData != null) {
              unit = distRoadData.road;
            }

            if (unit instanceof simpplle.comcode.element.Roads) {
              simpplle.comcode.element.Roads road = (simpplle.comcode.element.Roads)unit;
          ...
          if (ts == 0  || trailsDone) {
            ...
            if (unit instanceof simpplle.comcode.element.Trails) {
              simpplle.comcode.element.Trails trail = (simpplle.comcode.element.Trails)unit;
 ...
 */
  public String getMultipleHistory() throws SimpplleError {
    ...
    HashMap<SimpplleType, simpplle.comcode.utility.MyInteger> speciesFreq;
    HashMap<SimpplleType, simpplle.comcode.utility.MyInteger> sizeClassFreq;
    HashMap<SimpplleType, simpplle.comcode.utility.MyInteger> densityFreq;
    HashMap<SimpplleType, simpplle.comcode.utility.MyInteger> processFreq;
    ...
    fout.print(simpplle.comcode.utility.Formatting.fixedField("Species",22,true));
    fout.print(simpplle.comcode.utility.Formatting.fixedField("Size Class",23,true));
    fout.print(simpplle.comcode.utility.Formatting.fixedField("Density",17,true));
    fout.println(simpplle.comcode.utility.Formatting.fixedField("Process",26,true));

    fout.print(simpplle.comcode.utility.Formatting.fixedField("Frequencies",22,true));
    fout.print(simpplle.comcode.utility.Formatting.fixedField("Frequencies",23,true));
    fout.print(simpplle.comcode.utility.Formatting.fixedField("Frequencies",17,true));
    fout.println(simpplle.comcode.utility.Formatting.fixedField("Frequencies",26,true));
    ...
    for(i=0;i<numData;i++) {
      if (speciesIt.hasNext()) {
        ...
        fout.print(simpplle.comcode.utility.Formatting.fixedField(key.toString(),16,true));
        fout.print(simpplle.comcode.utility.Formatting.fixedField(freq.intValue(),4) + "% ");
      }
      else {
        fout.print(simpplle.comcode.utility.Formatting.fixedField("",22,true));
      }
      if (sizeClassIt.hasNext()) {
        key  = (SimpplleType)sizeClassIt.next();
        freq = sizeClassFreq.get(key);
        fout.print(simpplle.comcode.utility.Formatting.fixedField(key.toString(),17,true));
        fout.print(simpplle.comcode.utility.Formatting.fixedField(freq.intValue(),4) + "% ");
      }
      else {
        fout.print(simpplle.comcode.utility.Formatting.fixedField("",23,true));
      }
      if (densityIt.hasNext()) {
        key  = (SimpplleType)densityIt.next();
        freq = densityFreq.get(key);
        fout.print(simpplle.comcode.utility.Formatting.fixedField(key.toString(),11,true));
        fout.print(simpplle.comcode.utility.Formatting.fixedField(freq.intValue(),4) + "% ");
      }
      else {
        fout.print(simpplle.comcode.utility.Formatting.fixedField("",17,true));
      }
      if (processIt.hasNext()) {
        key  = (SimpplleType)processIt.next();
        freq = processFreq.get(key);
        fout.print(simpplle.comcode.utility.Formatting.fixedField(key.toString(),20,true));
        fout.print(simpplle.comcode.utility.Formatting.fixedField(freq.intValue(),4) + "%");
      }
      else {
        fout.print(simpplle.comcode.utility.Formatting.fixedField("",26,true));
      }
      fout.println();
    }
 ...
 */
  public String getTreatmentHistory() {
    ...
    fout.print(simpplle.comcode.utility.Formatting.fixedField("",5));
    fout.println("State Change");

    for(step=0;step<=nSteps;step++) {
      fout.print(simpplle.comcode.utility.Formatting.fixedField(step,5,true));
 ...
 */
  public static String getProbabilitySaveString(int prob) {
    switch (prob) {
      ...
      default:         return simpplle.comcode.utility.IntToString.get(prob);
    }
  }
/**
  ...
  public void doNextStateMultipleLifeform() {
    ...
    for (int i=0; i<nextStateToDo.size(); i++) {
      ...
      if (canDoFireRegen(processType,Simpplle.getCurrentZone())) {
        int delay = simpplle.comcode.logic.RegenerationDelayLogic.getInstance().getDelay(this,Area.currentLifeform);
        if (delay > 0) {
...
//  }
                                         ...
                                         RegenTypes regenType)
  {
    ...
    boolean producing = simpplle.comcode.logic.ProducingSeedLogic.getInstance().isSeedProducing(state,this,tStep,lifeform,regenType);
  ...
  public boolean weedWillSpread(Evu adj) {
    ...
    else if ((getRoadStatusNew() == simpplle.comcode.element.Roads.Status.OPEN &&
              adj.getRoadStatusNew() == simpplle.comcode.element.Roads.Status.OPEN) &&
 ...
 */
  public void readExternal(ObjectInput in) throws IOException, ClassNotFoundException {
    ...
    if (version > 7) {
      roadStatus = (simpplle.comcode.element.Roads.Status)in.readObject();
    }
    else {
      roadStatus = simpplle.comcode.element.Roads.Status.lookup(in.readInt());
    }
 ...
 */
    ...
    if (version > 0) {
      {
        int size = in.readInt();
        if (size > 0) {
          assocRoadUnits = new ArrayList<simpplle.comcode.element.Roads>(size);
          for (int i = 0; i < size; i++) {
...

      {
        int size = in.readInt();
        if (size > 0) {
          assocTrailUnits = new ArrayList<simpplle.comcode.element.Trails>(size);
          for (int i = 0; i < size; i++) {
  ...
  public void writeSpatialRelations(ObjectOutput out) throws IOException {
    ...
    if (assocRoadUnits != null) {
      for (simpplle.comcode.element.Roads road : assocRoadUnits) {
    ...
    if (assocTrailUnits != null) {
      for (simpplle.comcode.element.Trails trail : assocTrailUnits) {
