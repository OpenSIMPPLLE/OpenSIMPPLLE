 ...
 *
 * <p>This class contains fields and method relating to Existing Aquatic States.  The class is utilized to provide a water source for bison grazing logic. 
...
//      return htGrp.getVegetativeType("ND/ND/1");
  ...
  public float getFloatLength() {
    return ( (float)length / (float) simpplle.comcode.utility.Utility.pow(10,Area.getLengthPrecision()) );
  }

  public void setLength(int val) { length = val; }
  public void setLength(float val) {
    length = Math.round(val * simpplle.comcode.utility.Utility.pow(10,Area.getLengthPrecision()));
  }
 ...
 */
  ...
  public void addAdjacentEvu(simpplle.comcode.element.Evu unit) {
  ...
  public void addUplandEvu(simpplle.comcode.element.Evu unit) {
  ...
  public void readAdjacentEvus (simpplle.comcode.utility.MyStringTokenizer mainStrTok) throws ParseError, IOException {
    adjacentEvus = readEvus(mainStrTok);
  }
  public void readUplandEvus (simpplle.comcode.utility.MyStringTokenizer mainStrTok) throws ParseError, IOException {
    uplandEvus = readEvus(mainStrTok);
  }

  private ArrayList readEvus (simpplle.comcode.utility.MyStringTokenizer mainStrTok) throws ParseError, IOException {
    ...
    strTok = new simpplle.comcode.utility.StringTokenizerPlus(str,COLON);
  ...
  public void readPredecessors(simpplle.comcode.utility.MyStringTokenizer mainStrTok) throws ParseError, IOException {
    predecessors = readAquaticUnits(mainStrTok);
  }
  public void readSuccessors(simpplle.comcode.utility.MyStringTokenizer mainStrTok) throws ParseError, IOException {
    successors = readAquaticUnits(mainStrTok);
  }

  private ArrayList readAquaticUnits (simpplle.comcode.utility.MyStringTokenizer mainStrTok) throws ParseError, IOException {
    simpplle.comcode.utility.StringTokenizerPlus strTok;
    int                   id;
  ...
  private void readDelimitedData(simpplle.comcode.utility.MyStringTokenizer strTok)
  ...
  public void exportNeighborsVegetation(PrintWriter fout) {
    ...
    if (uplandEvus != null) {
      for (i = 0; i < uplandEvus.size(); i++) {
        fout.print(getId());
        fout.print(COMMA);
        fout.print(((simpplle.comcode.element.Evu)uplandEvus.get(i)).getId());
        fout.print(COMMA);
        fout.println("U");
      }
    }
    ...
    if (adjacentEvus != null) {
      for (i = 0; i < adjacentEvus.size(); i++) {
        fout.print(getId());
        fout.print(COMMA);
        fout.print(((simpplle.comcode.element.Evu)adjacentEvus.get(i)).getId());
        fout.print(COMMA);
        fout.println("A");
      }
    }
  ...
  public String[] getUplandVegDisplay() {
    ...
    for(int i=0;i<uplandEvus.size();i++) {
      strList[i] = Integer.toString(((simpplle.comcode.element.Evu)uplandEvus.get(i)).getId()) + "      ";
    }
    return strList;
  }
  public String[] getAdjVegDisplay() {
    ...
    for(int i=0;i<adjacentEvus.size();i++) {
      strList[i] = Integer.toString(((simpplle.comcode.element.Evu)adjacentEvus.get(i)).getId()) +
                              "      ";
    }
    return strList;
  }
 ...
 */
  ...
  public void writeExternal(ObjectOutput out) throws IOException {
    ...
    if (uplandEvus == null) {
      ...
      for (Object item : uplandEvus) {
        out.writeInt(((simpplle.comcode.element.Evu) item).getId());
      }
    }
  ...
  public float getFloatProb(int tStep) {
    return ( (float)getProb(tStep) / (float) simpplle.comcode.utility.Utility.pow(10,Area.getLengthPrecision()) );
  }
