 ...
 *
 * <p>This class contains methods for Natural Elements.  Natural elements are EVU, EAU, and ELU.  
  ...
  public void readExternal(ObjectInput in) throws IOException, ClassNotFoundException {
    ...
    if (version == 1) {
      ...
      for (int i=0; i<size; i++) {
        int id = in.readInt();
        if (this instanceof simpplle.comcode.element.ExistingLandUnit) {
          simpplle.comcode.element.ExistingLandUnit unit = area.getElu(id);
          if (unit == null) {
            unit = new simpplle.comcode.element.ExistingLandUnit(id);
            area.addElu(unit);
          }
          neighbors.add(unit);
        }
        else if (this instanceof simpplle.comcode.element.Evu) {
          simpplle.comcode.element.Evu unit = area.getEvu(id);
          if (unit == null) {
            unit = new simpplle.comcode.element.Evu(id);
            area.addEvu(unit);
          }
          neighbors.add(unit);
        }
