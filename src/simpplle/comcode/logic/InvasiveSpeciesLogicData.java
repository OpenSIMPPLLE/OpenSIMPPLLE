 ...
 *
 * <p>This class contains methods to handle Invasive Species LogicData, a type of Logic Data.
 * 
 ...
 * @return
 */
  public boolean isMatch(simpplle.comcode.element.Evu evu, ArrayList<simpplle.comcode.element.Evu> invasiveUnits) {
    if (super.isMatch(evu) == false) { return false; }

    { // *** Soil Type ***
      boolean aMatch = false;
      if (soilTypeList != null && soilTypeList.size() > 0) {
        ArrayList<simpplle.comcode.element.ExistingLandUnit> elus = evu.getAssociatedLandUnits();
    ...
    { // *** Dist to Seed Source ***
      if (distToSeed > 0 && invasiveUnits.size() > 0) {
        ...
        simpplle.comcode.element.Evu closestEvu;
 ...
 */
  public Object getValueAt(int col) {
    switch (col) {
      case simpplle.comcode.logic.InvasiveSpeciesLogic.INVASIVE_SPECIES_COL: return invasiveSpeciesDesc;
      case simpplle.comcode.logic.InvasiveSpeciesLogic.SOIL_TYPE_COL:      return soilTypeDesc;
      case simpplle.comcode.logic.InvasiveSpeciesLogic.VEG_FUNC_GROUP_COL: return vegFuncGroup;
      case simpplle.comcode.logic.InvasiveSpeciesLogic.DIST_TO_SEED_COL:   return distToSeed;
      case simpplle.comcode.logic.InvasiveSpeciesLogic.START_VALUE_COL:    return startValue;
      case simpplle.comcode.logic.InvasiveSpeciesLogic.PROB_COL:           return prob;
      default:
  ...
  public void setValueAt(int col, Object value) {
    switch (col) {
      case simpplle.comcode.logic.InvasiveSpeciesLogic.INVASIVE_SPECIES_COL: break; // set in table editor;
      case simpplle.comcode.logic.InvasiveSpeciesLogic.SOIL_TYPE_COL:     break; // set in table editor;
      // Can't test equality of double so always mark as changed.
      case simpplle.comcode.logic.InvasiveSpeciesLogic.VEG_FUNC_GROUP_COL:
        if (vegFuncGroup != (VegFunctionalGroup)value) {
          vegFuncGroup = (VegFunctionalGroup) value;
          SystemKnowledge.markChanged(sysKnowKind);
        }
        break;
      case simpplle.comcode.logic.InvasiveSpeciesLogic.DIST_TO_SEED_COL:
        distToSeed = (Long)value;
        SystemKnowledge.markChanged(sysKnowKind);
        break;
      case simpplle.comcode.logic.InvasiveSpeciesLogic.START_VALUE_COL:
        if (startValue != (Integer)value) {
          startValue = (Integer)value;
          SystemKnowledge.markChanged(sysKnowKind);
        }
        break;
      case simpplle.comcode.logic.InvasiveSpeciesLogic.PROB_COL:
