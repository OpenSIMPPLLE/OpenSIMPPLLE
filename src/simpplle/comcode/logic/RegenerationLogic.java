...
public abstract class RegenerationLogic {
  ...
  private static HashMap<HabitatTypeGroupType, simpplle.comcode.logic.BaseLogic> fireData =
      new HashMap<HabitatTypeGroupType, simpplle.comcode.logic.BaseLogic>();
  private static HashMap<HabitatTypeGroupType, simpplle.comcode.logic.BaseLogic> succData =
      new HashMap<HabitatTypeGroupType, simpplle.comcode.logic.BaseLogic>();
 ...
 * @return
 */
  private static HashMap<HabitatTypeGroupType, simpplle.comcode.logic.BaseLogic> getDataHm(DataKinds kind) {
    switch (kind) {
 ...
 */
   ...
   * @return
   */
  public static simpplle.comcode.logic.BaseLogic getLogicInstance(String kind) {
    return getData(DataKinds.valueOf(kind));
  }
  public static simpplle.comcode.logic.BaseLogic getData(DataKinds kind, HabitatTypeGroupType ecoGroup) {
   ...
   * @return
   */
  public static simpplle.comcode.logic.BaseLogic getData(DataKinds kind, HabitatTypeGroupType ecoGroup, boolean addIfNull) {
    simpplle.comcode.logic.BaseLogic result;
    switch (kind) {
      case FIRE:
        result = fireData.get(ecoGroup);
        if (result == null && addIfNull) {
          fireData.put(ecoGroup,new simpplle.comcode.logic.RegenBaseLogic(kind));
        }
        return fireData.get(ecoGroup);
      case SUCCESSION:
        result = succData.get(ecoGroup);
        if (result == null && addIfNull) {
          succData.put(ecoGroup,new simpplle.comcode.logic.RegenBaseLogic(kind));
        }
  ...
  public static simpplle.comcode.logic.BaseLogic getData(DataKinds kind) {
    return getData(kind,false);
  }
  public static simpplle.comcode.logic.BaseLogic getData(DataKinds kind, boolean addIfNull) {
 ...
 */
  ...
  public static void setSpecies(int row, Species species, DataKinds kind) {
    simpplle.comcode.logic.RegenerationData regenData =
      (simpplle.comcode.logic.RegenerationData)getData(kind).getValueAt(row,kind.toString());
  ...
  public static boolean isSpeciesPresent(Species species, DataKinds kind) {
    simpplle.comcode.logic.BaseLogic logic = getData(kind);
    ArrayList<simpplle.comcode.logic.AbstractLogicData> dataList = logic.getData(kind.toString());
    if (dataList != null) {
      for (simpplle.comcode.logic.AbstractLogicData data : dataList) {
        simpplle.comcode.logic.RegenerationData regenData = (simpplle.comcode.logic.RegenerationData)data;
  ...
  private static simpplle.comcode.logic.RegenerationData findRegenData(HabitatTypeGroupType ecoGroup,
   ...
   */
  private static simpplle.comcode.logic.RegenerationData findRegenData(HabitatTypeGroupType ecoGroup,
                                                                       simpplle.comcode.element.Evu evu, int tStep, Lifeform lifeform,
                                                                       DataKinds kind) {
    simpplle.comcode.logic.BaseLogic logic = getData(kind,ecoGroup);
    if (logic != null) {
      ArrayList<simpplle.comcode.logic.AbstractLogicData> dataList = logic.getData(kind.toString());
      if (dataList != null) {
        for (simpplle.comcode.logic.AbstractLogicData data : dataList) {
  ...
  private static simpplle.comcode.logic.RegenerationData findRegenDataSuccInLandscapeSeed(
      HabitatTypeGroupType ecoGroup, simpplle.comcode.element.Evu evu,
  ...
  public static ArrayList<VegetativeType> getAdjacentStates(HabitatTypeGroupType ecoGroup, simpplle.comcode.element.Evu evu,
                                                            int tStep, Lifeform lifeform) {
    FireRegenerationData regenData =
       (FireRegenerationData)findRegenData(ecoGroup,evu,tStep,lifeform,FIRE);
...
//      setFile(infile);
    ...
    throws ParseError, IOException
  {
    ...
    ArrayList<simpplle.comcode.logic.RegenerationData> dataList;
    ArrayList<simpplle.comcode.logic.RegenerationData> succDataList;
...
//      regenData.setAdjResprouting(null);
      ...
      {
        simpplle.comcode.logic.BaseLogic logic = getData(FIRE, ecoGroup,true);
        logic.addRow(FIRE.toString(), regenData);
      }
      {
        simpplle.comcode.logic.BaseLogic logic = getData(SUCCESSION, ecoGroup,true);
  ...
  private static void sortData(ArrayList<simpplle.comcode.logic.AbstractLogicData> dataList) {
    ...
    for (int i=0; i<dataList.size(); i++) {
      regenData = (simpplle.comcode.logic.RegenerationData)dataList.get(i);
    ...
    for (int i=0; i<allLives.length; i++) {
      ...
      for (int j=0; j<list.size(); j++) {
        dataList.add((simpplle.comcode.logic.RegenerationData)list.get(j));
      }
      list.clear();
      list = null;
    }
    ...
    throws IOException, ClassNotFoundException
 {
    ...
    throws IOException, ClassNotFoundException
  {
    ...
    for (int i=0; i<size; i++) {
      ...
      simpplle.comcode.logic.BaseLogic logic = getData(kind,ecoGroup,true);
      logic.read(in);
    }
  }
  ...
  public static void duplicateRow(int row,int insertPos, DataKinds kind) {
    simpplle.comcode.logic.AbstractLogicData logicData, newLogicData;
