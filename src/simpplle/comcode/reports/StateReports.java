...
public final class StateReports extends Reports {
  ...
  public String getStateReport(Types kind, boolean combineLifeforms) {
  ...
  private String getSingleStateReport(Types kind, boolean combineLifeforms) {
    PrintWriter  fout;
  ...
  private void stateReport(PrintWriter fout, Types kind, boolean combineLifeforms) {
    ...
    fout.println();
    fout.println(simpplle.comcode.utility.Formatting.padLeft("ACRES OF " + kindName + " (rounded)",31));
    ...
    for(int i=0;i<=timeSteps;i += 5) {
      ...
      fout.print(simpplle.comcode.utility.Formatting.padLeft(kindName,11));
      ...
      for(int j=i;j<=lastTime;j++) {
        fout.print(simpplle.comcode.utility.Formatting.fixedField(j,10));
      }
      fout.println();
      fout.println();
      ...
      for (String key : dataHm.keySet()) {
        ...
        for(int k=i;k<=lastTime;k++) {
          ...
          fout.print(simpplle.comcode.utility.Formatting.fixedField(nf.format(acres),10));
        }
        fout.println();
      }
      fout.println();
  ...
  private void stateReport(PrintWriter fout, SimpplleType.Types stateKind, int kind,
    ...
    for (String key : dataHm.keySet()) {
      ...
      for(int i=0;i<=timeSteps;i += 5) {
        ...
        fout.print(simpplle.comcode.utility.Formatting.padLeft(stateKind.toString(),11));
        fout.println(simpplle.comcode.utility.Formatting.padLeft("time",38));
        ...
        for (String stateKey : innerDataHm.keySet()) {
          ...
          fout.print(simpplle.comcode.utility.Formatting.fixedField(stateKey.toString(),41,true));
